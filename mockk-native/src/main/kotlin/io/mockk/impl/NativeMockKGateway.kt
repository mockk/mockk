...
import io.mockk.impl.instantiation.NativeInstantiator
import io.mockk.impl.instantiation.NativeMockFactory
import io.mockk.impl.log.NativeConsoleLogger
...
class NativeMockKGateway : MockKGateway {
    ...
    val instantiator = NativeInstantiator(instanceFactoryRegistryIntrnl)
    val anyValueGenerator = AnyValueGenerator()
    val signatureValueGenerator = NativeSignatureValueGenerator()


    override val mockFactory: MockFactory = NativeMockFactory(
        stubRepo,
        instantiator,
    ...
    companion object {
        ...
        init {
            Logger.loggerFactory = { cls: KClass<*> -> NativeConsoleLogger(cls) }

            log = Logger<NativeMockKGateway>()

            log.trace {
                "Starting Native MockK implementation. "
            }
        }

        val defaultImplementation = NativeMockKGateway()
