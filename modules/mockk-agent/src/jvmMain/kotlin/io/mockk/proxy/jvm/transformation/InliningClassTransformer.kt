...
internal class InliningClassTransformer(
    ...
    /**
     * Byte Buddy ignores synthetic methods by default. When redefining a class that only drops
     * non-virtual ones from the methods handed to the advice visitors, so a static method flagged
     * `ACC_SYNTHETIC` is never instrumented and `mockkStatic` cannot stub it. Kotlin flags
     * top-level functions and `@JvmStatic` members annotated with `@JvmSynthetic` that way.
     *
     * Static helpers generated by the compilers stay ignored: `foo$default`, `access$foo`, lambda
     * bodies and value class `-impl` methods. Recording those instead of the function they belong to
     * would break matching, e.g. `every { foo() }` would stub `foo$default` and miss `foo(5)`.
     * The compilers put `$` or `-` in those names, which hand-written function names rarely contain.
     */
    private val redefiningByteBuddy =
        byteBuddy.ignore(
            isSynthetic<MethodDescription>()
                .and(
                    not(
                        isStatic<MethodDescription>()
