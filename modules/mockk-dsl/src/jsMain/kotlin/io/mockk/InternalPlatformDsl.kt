...
actual object InternalPlatformDsl {
    ...
    actual fun dynamicGet(
        self: Any,
        name: String,
    ): Any? = throw MockKException("dynamic get is not supported on JS platform")

    actual fun dynamicSet(
        self: Any,
        name: String,
        value: Any?,
    ): Unit = throw MockKException("dynamic set is not supported on JS platform")
    ...
    actual fun <T> coroutineCall(lambda: suspend () -> T): CoroutineCall<T> = throw MockKException("coroutineCall is not supported")
}
