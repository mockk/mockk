...
open class UnorderedCallVerifier(
    ...
    private fun matchCall(
        ...
        if (matchedCalls.size > 1 && matcher.args.any { it is CapturingSlotMatcher<*> }) {
            val msg =
                "$matcher execution is being verified more than once and its arguments are being " +
                    "captured with a slot.\n" +
        ...
        val result =
            if (min == 0 && max == 0) {
                ...
                when (allCallsForMockMethod.size) {
                    0 -> {
                        if (min == 0) {
                            VerificationResult.OK(listOf())
                    ...
                    1 -> {
                        ...
                        if (matchedCalls.size == 1) {
                            ...
                            if (0 in min..max) {
                                ...
                                VerificationResult.Failure(
                                    safeToString.exec {
                                        "$callIdxMsg. Only one matching call to ${stub.toStr()}/${matcher.method.toStr()} happened, " +
