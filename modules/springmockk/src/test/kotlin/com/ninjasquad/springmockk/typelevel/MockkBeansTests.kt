...
/**
 * Tests for [@MockkBeans][MockkBeans]: [@MockkBean][MockkBean]
 ...
 */
class MockkBeansTests {
    @Test
    fun registrationOrderForTopLevelClass() {
        val mockedServices = getRegisteredMockTypes(MockkBeansByTypeIntegrationTests::class.java)
    ...
    fun registrationOrderForNestedClass() {
        val mockedServices =
            getRegisteredMockTypes(MockkBeansByTypeIntegrationTests.NestedTests::class.java)
        assertThat(mockedServices).containsExactly(
            Service01::class.java, Service02::class.java, Service03::class.java, Service04::class.java,
    ...
    companion object {
        private fun getRegisteredMockTypes(testClass: Class<*>): Stream<Class<*>> {
            return BeanOverrideTestUtils.findAllHandlers(testClass)
                .stream()
                .map({ obj -> obj.getBeanType() })
                .map({ obj -> obj.getRawClass() })
        }
    }
}
