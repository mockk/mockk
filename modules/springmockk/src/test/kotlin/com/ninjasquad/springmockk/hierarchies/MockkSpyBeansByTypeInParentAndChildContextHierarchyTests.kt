...
import com.ninjasquad.springmockk.hierarchies.MockkSpyBeansByTypeInParentAndChildContextHierarchyTests.Config1
import com.ninjasquad.springmockk.hierarchies.MockkSpyBeansByTypeInParentAndChildContextHierarchyTests.Config2
...
/**
 * Verifies that [@MockkSpyBean][MockkSpyBean] can be used within a
 ...
 * configured via class-level `@MockkSpyBean` declarations.
 *
 * @author Sam Brannen
 * @since 6.2.6
 */
...
@MockkSpyBean(types = [ExampleService::class], contextName = "parent")
@MockkSpyBean(types = [ExampleService::class], contextName = "child")
class MockkSpyBeansByTypeInParentAndChildContextHierarchyTests {
    @Autowired
    ...
    fun test(context: ApplicationContext) {
        val serviceInParent: ExampleService = context.parent!!.getBean<ExampleService>(ExampleService::class.java)
