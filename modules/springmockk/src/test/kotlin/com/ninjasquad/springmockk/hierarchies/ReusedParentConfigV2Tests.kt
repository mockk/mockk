 ...
 * [ReusedParentConfigV1Tests] will fail when both test classes are run
...
class ReusedParentConfigV2Tests {
    ...
    fun test(context: ApplicationContext) {
        assertThat(context.parent!!.getBeanNamesForType(BarService::class.java)).isEmpty()
        assertThat(context.getBeanNamesForType(BarService::class.java)).hasSize(1)

        every { barService.bar() } returns "mock"
