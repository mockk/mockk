...
/**
 * Integration tests for [MockkSpyBean] that use by-name lookup with test class
...
class MockkSpyBeanByNameLookupTestClassScopedExtensionContextIntegrationTests {
    @MockkSpyBean("field1")
    ...
    fun fieldForPrototypeHasOverride(ctx: ConfigurableApplicationContext) {
        ...
        assertThat(ctx.getBeanFactory().getBeanDefinition("field3").isSingleton()).`as`("isSingleton")
            .isTrue()
    ...
    class Config {
        ...
        fun bean3(): ExampleService? {
            return RealExampleService("bean3")
        }
    }
}
