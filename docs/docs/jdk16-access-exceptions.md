...
On JDK 16 and above you may encounter `InaccessibleObjectException` or `IllegalAccessException` in the following known
cases:
...
2. Spying on Java standard library classes, e.g.
   ...
   ```
   This includes creating `@SpyKBean` on spring data repository instances, because they are instances of
   `java.lang.reflect.Proxy` in fact.
...
JDK 16 enforces strong encapsulation of standard modules. In practice this mean overriding accessibility modifiers in
JDK classes (for example,  `privateMember.setAccessible(true)`) is forbidden.
...
Add JVM argument `--add-opens java.base/full.package.name=ALL-UNNAMED` _for each package_ you need to mock. Package name
